...
def plot_results(vanilla_sgd_log, fedavg_log, lasg_wk2_log, target_test_accuracy, iid, dataset, modeltype, mode="show"):
    """
    ...
    dataset : str
        name of the dataset being used in the experiments.
    modeltype : str
        name of the model type being used in the experiments.
    mode : str
        Can be "show" to show the plots or "save" to save them as png.
    ...
    save_path = "{0}_{1}_{2}".format(dataset, modeltype, iid_label)
    if mode=="save" and not os.path.exists(save_path):
        os.mkdir(save_path)
    ...
    axes[1].legend()
    if mode == "save":
        plt.savefig("{0}/loss.png".format(save_path))
    else:
        plt.show()
    ...
    axes[1].legend()
    if mode == "save":
        plt.savefig("{0}/test_accuracy.png".format(save_path))
    else:
        plt.show()
    ...
    ax.legend()
    if mode == "save":
        plt.savefig("{0}/uploads_fraction.png".format(save_path))
    else:
        plt.show()
        
    if mode == "save":
        with open("{0}/vanilla_sgd_log.json".format(save_path), "w") as file:
            json.dump(vanilla_sgd_log, file, indent=4)
            
        with open("{0}/fedavg_log.json".format(save_path), "w") as file:
            json.dump(fedavg_log, file, indent=4)
            
        with open("{0}/lasg_wk2_log.json".format(save_path), "w") as file:
            json.dump(lasg_wk2_log, file, indent=4)